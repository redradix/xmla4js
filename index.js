...
*/
     ...
     *   @return {DOMDocument}
     */
    getResponseXML: function (normalize) {
      if (_isObj(this.responseXML)) return this.responseXML
      return (this.responseXML = _xjs(this.responseText, normalize))
    },
    /**
...
*/
      ...
      switch (n) {
        ...
        case 1:
          ...
          if (_isObj(arg)) {
            ...
            else if ((events = arg.events || arg.event)) {
              if (_isStr(events))
                events = events === 'all' ? Xmla.EVENT_ALL : events.split(',')
              if (!_isArr(events)) {
                Xmla.Exception._newError(
                  'WRONG_EVENTS_FORMAT',
                  'Xmla.addListener',
        ...
        case 3:
          ...
          scope = arguments[2]
          handler = arguments[1]
          if (_isStr(event) && (_isFun(handler) || _isObj(handler)))
            this._addListener(event, handler, scope)
          else {
    ...
    _addListener: function (name, handler, scope) {
      ...
      if (!myListeners)
        Xmla.Exception._newError('UNKNOWN_EVENT', 'Xmla.addListener', {
          event: name,
          handler: handler,
          scope: scope,
        })._throw()
    ...
    _fireEvent: function (eventName, eventData, cancelable) {
      ...
      } //if there is neither a listener nor an error nor a general callback  we explicitly throw the exception.
      else if (
        eventName === Xmla.EVENT_ERROR &&
        !_isFun(eventData.error) &&
        !_isFun(eventData.callback)
      )
    ...
    request: function (options) {
      ...
      options.properties = _applyProps(
        options.properties,
        this.options.properties,
        false,
      )
      options.restrictions = _applyProps(
        options.restrictions,
      ...
      if (
        ...
        ((options.method == Xmla.METHOD_DISCOVER &&
          this._fireEvent(Xmla.EVENT_DISCOVER, options)) ||
          (options.method == Xmla.METHOD_EXECUTE &&
            this._fireEvent(Xmla.EVENT_EXECUTE, options)))
      )
    ...
    _requestSuccess: function (request) {
      ...
      try {
        var responseXml = this.getResponseXML(request.normalize)
        var soapFault = _getElementsByTagNameNS(
          responseXml,
          _xmlnsSOAPenvelope,
          _xmlnsSOAPenvelopePrefix,
          'Fault',
        ...
        if (soapFault.length) {
          ...
          switch (method) {
            case Xmla.METHOD_DISCOVER:
              request.rowset = response = new Xmla.Rowset(
                responseXml,
                request.requestType,
                this,
              )
            ...
            case Xmla.METHOD_EXECUTE:
              ...
              switch (format) {
                case Xmla.PROP_FORMAT_TABULAR:
                  response = resultset = new Xmla.Rowset(
                    responseXml,
                    null,
                    this,
                  )
...
*/
  ...
  Xmla.Rowset.KEYS[Xmla.MDSCHEMA_CUBES] = [
    'CATALOG_NAME',
    'SCHEMA_NAME',
    'CUBE_NAME',
  ]
  Xmla.Rowset.KEYS[Xmla.MDSCHEMA_DIMENSIONS] = [
    'CATALOG_NAME',
    'SCHEMA_NAME',
    'CUBE_NAME',
  ...
  Xmla.Rowset.KEYS[Xmla.MDSCHEMA_HIERARCHIES] = [
    'CATALOG_NAME',
    'SCHEMA_NAME',
    'CUBE_NAME',
    'DIMENSION_UNIQUE_NAME',
  ...
  Xmla.Rowset.KEYS[Xmla.MDSCHEMA_MEASURES] = [
    'CATALOG_NAME',
    'SCHEMA_NAME',
    'CUBE_NAME',
    'MEASURE_NAME',
  ...
  function _getterNameForColumnName(columnName) {
    ...
    return (
      'get' +
      (/^[A-Z]+[a-z]+[A-Za-z]*$/g.test(columnName)
        ? columnName
        : columnName.charAt(0).toUpperCase() +
          columnName
            .substr(1)
            .toLowerCase()
            .replace(/_[a-z]/g, function (a) {
  ...
  Xmla.Rowset.prototype = {
    ...
    _fieldCount: null,
    _initData: function () {
      this._rows = _getElementsByTagNameNS(
        this._node,
        _xmlnsRowset,
        null,
        'row',
      ...
      if (rowSchema) {
        var seq = _getElementsByTagNameNS(
            rowSchema,
            _xmlnsSchema,
            _xmlnsSchemaPrefix,
            'sequence',
          )[0],
          seqChildren = seq.childNodes,
          numChildren = seqChildren ? seqChildren.length : 0,
          seqChild,
          fieldLabel,
          fieldName,
          minOccurs,
          ...
          valueConverter,
          getter,
          addFieldGetters = this._xmla.options.addFieldGetters,
          i,
          val
        for (i = 0; i < numChildren; i++) {
          seqChild = seqChildren[i]
          if (seqChild.nodeType !== 1) continue
          fieldLabel = _getAttributeNS(
            seqChild,
            _xmlnsSQL,
            _xmlnsSQLPrefix,
            'field',
          ...
          if (type === null && this._row) {
            //bummer, not defined there try to get it from xsi:type in the row
          ...
          getter = this._createFieldGetter(
            fieldName,
            valueConverter,
            minOccurs,
            maxOccurs,
...
*/
    ...
    mapAsObject: function (map, key, row) {
      ...
      for (i = 0; i < len; i++) {
        ...
        if (p) {
          if (i === last) {
            //last, we need to store the row now.
        ...
        } //property didnt exist for this key yet.
        else if (i === last) m[v] = row //last keypart: store the row here
  ...
  Xmla.Dataset.prototype = {
    ...
    _initDataset: function (doc) {
      ...
      this.cubeName = _getElementText(
        _getElementsByTagNameNS(this._root, _xmlnsDataset, '', 'CubeName')[0],
      )
      this._initAxes()
      this._initCells()
      ...
      func =
        ...
        '\nif (arguments.length !== ' +
        this._numAxes +
        ') new Xmla.Exception._newError("ERROR_ILLEGAL_ARGUMENT", "cellOrdinalForTupleIndexes", this)._throw();'
      for (a = 0, i = this._numAxes - 1; i >= 0; i--, a++) {
        func +=
          '\nif (typeof(a = arguments[' +
          a +
          '])!=="number") new Xmla.Exception._newError("ERROR_ILLEGAL_ARGUMENT", "cellOrdinalForTupleIndexes", this)._throw();'
        mul = 1
      ...
      func += funcBody + '\nreturn ordinal;'
      this._cellset.cellOrdinalForTupleIndexes =
        this.cellOrdinalForTupleIndexes = new Function(func)
    },
    ...
    _initAxes: function () {
      ...
      axisNodes = _getElementsByTagNameNS(
        this._root,
        _xmlnsDataset,
        '',
        'AxisInfo',
    ...
    _getAxis: function (nameOrIndex) {
      ...
      switch (typeof nameOrIndex) {
        ...
        case 'string':
          axis =
            name === Xmla.Dataset.AXIS_SLICER ? this._slicer : this._axes[name]
          break
      }
      return axis
    },
  ...
  Xmla.Dataset.Axis.prototype = {
    ...
    _initMembers: function () {
      var root = this._dataset._root,
        memberSchema,
        memberSchemaElements,
        numMemberSchemaElements,
        memberSchemaElement,
        type,
        name,
        i,
      ...
      ;(memberSchema = _getElementsByTagNameNS(
        memberSchema,
        _xmlnsSchema,
        _xmlnsSchemaPrefix,
        'sequence',
      )[0]),
        (memberSchemaElements = _getElementsByTagNameNS(
          memberSchema,
          _xmlnsSchema,
          _xmlnsSchemaPrefix,
          'element',
    ...
    _initAxis: function (_axisInfoNode, _axisNode) {
      ...
      this._tuples = _getElementsByTagNameNS(
        _axisNode,
        _xmlnsDataset,
        '',
        'Tuple',
    ...
    eachHierarchy: function (callback, scope, args) {
      ...
      while (this.hasMoreHierarchies()) {
        mArgs[0] =
          this._hierarchyDefs[this._hierarchyOrder[this._hierarchyIndex]]
    ...
    hierarchyName: function (index) {
      ...
      if (index !== parseInt(index, 10) || index >= this.numHierarchies)
        Xmla.Exception._newError(
          'INVALID_HIERARCHY',
          'Xmla.Dataset.Axis.hierarchyDef',
          index,
    ...
    hierarchy: function (hierarchyIndexOrName) {
      ...
      if (_isNum(hierarchyIndexOrName)) {
        if (
          hierarchyIndexOrName !== parseInt(hierarchyIndexOrName, 10) ||
    ...
    _member: function (index) {
      ...
      for (i = 0; i < n; i++) {
        el = childNodes[i]
        if (el.nodeType !== 1 || !(property = memberProperties[el.nodeName]))
          continue
    ...
    _initCellset: function () {
      ...
      for (i = 0; i < numPropertyNodes; i++) {
        ...
        for (j = 0; j < numCellSchemaElements; j++) {
          ...
          this['cell' + propertyNodeTagName] = new Function(
...
*/
    _readCell: function (node, object) {
      ...
      for (p in this._cellProperties) {
        cellProp = _getElementsByTagNameNS(node, _xmlnsDataset, '', p)[0]
    ...
    indexForOrdinal: function (ordinal) {
      ...
      while (true) {
        ...
        else if (cellOrdinal > ordinal) index--
        else return -1
      }
      return null
    },
    ...
    getByTupleIndexes: function () {
      return this.getByOrdinal(
        this.cellOrdinalForTupleIndexes.apply(this, arguments),
      )
    },
    /**
     *  Close this cellset.
...
*/
  ...
  Xmla.Exception.INVALID_FIELD_HLP =
    ...
    '_' +
    Xmla.Exception.INVALID_FIELD_MSG
  ...
  Xmla.Exception.INVALID_AXIS_HLP =
    ...
    '_' +
    Xmla.Exception.INVALID_AXIS_MSG
  ...
  Xmla.Exception.ERROR_INSTANTIATING_XMLHTTPREQUEST_MSG =
    'Error creating XML Http Request'
